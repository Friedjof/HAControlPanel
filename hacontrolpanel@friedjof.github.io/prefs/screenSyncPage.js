...
export const ScreenSyncPage = GObject.registerClass(
class ScreenSyncPage extends Adw.PreferencesPage {
    _init(settings) {
        super._init({
            title: 'Screen Sync',
        ...
        // Legacy: 'browser' scope was replaced by the browser-bridge-priority flag
        ...
        const createSpinActionRow = ({
            title, subtitle, lower, upper, step, page, value, digits = 0, unit = '', onChanged,
        }) => {
            const row = new Adw.ActionRow({ title, subtitle });
            const spin = new Gtk.SpinButton({
                adjustment: new Gtk.Adjustment({ lower, upper, step_increment: step, page_increment: page, value }),
                digits,
        ...
        const { row: outputIntervalRow, spin: outputIntervalSpin } = createSpinActionRow({
            ...
            lower: 100, upper: 2000, step: 50, page: 100,
        ...
        this._screenSyncModeDropdown = new Gtk.DropDown({ model: this._screenSyncModeModel, valign: Gtk.Align.CENTER });
        setDropDownValue(this._screenSyncModeDropdown, this._screenSyncModeModel, settings.get_string('screen-sync-mode') || 'dominant');
        ...
        // To add a mode: add an entry here AND add the interpolator in lib/sync/screenSyncController.js.
        ...
        this._screenSyncTransitionDropdown = new Gtk.DropDown({ model: this._screenSyncTransitionModel, valign: Gtk.Align.CENTER });
        ...
        const { row: thresholdRow, spin: thresholdSpin } = createSpinActionRow({
            ...
            lower: 0, upper: 255, step: 1, page: 10,
        ...
        if (gdkDisplay) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const posLabel = geo.x === 0 && geo.y === 0 ? '' : ` at (+${geo.x},+${geo.y})`;
                const label = name
        ...
        this._screenSyncScopeDropdown = new Gtk.DropDown({ model: this._screenSyncScopeModel, valign: Gtk.Align.CENTER });
        ...
        const { row: historySizeRow, spin: historySizeSpin } = createSpinActionRow({
            ...
            lower: 2, upper: 8, step: 1, page: 1,
            value: settings.get_int('screen-sync-history-size') || 4,
            digits: 0, unit: 'samples',
        ...
        const { row: emaTimeRow, spin: emaTimeSpin } = createSpinActionRow({
            ...
            lower: 0.1, upper: 10, step: 0.1, page: 0.5,
        ...
        const { row: springStiffnessRow, spin: springStiffnessSpin } = createSpinActionRow({
            ...
            lower: 0.01, upper: 1.0, step: 0.01, page: 0.05,
        ...
        const { row: springDampingRow, spin: springDampingSpin } = createSpinActionRow({
            ...
            lower: 0.05, upper: 0.99, step: 0.01, page: 0.05,
        ...
        this._screenSyncConditionEnabledRow.connect('notify::active', () => {
            ...
            this._saveSyncCondition(settings, { ...current, enabled: this._screenSyncConditionEnabledRow.active });
        ...
        this._screenSyncConditionEntityRow.connect('changed', () => {
            const current = this._loadSyncCondition(settings);
            this._saveSyncCondition(settings, { ...current, entity_id: this._screenSyncConditionEntityRow.text });
            this._updateScreenSyncConditionActions();
        });
        ...
        this._screenSyncConditionOperatorDropdown = new Gtk.DropDown({
            model: this._screenSyncConditionOperatorModel, valign: Gtk.Align.CENTER,
        });
        setDropDownValue(this._screenSyncConditionOperatorDropdown, this._screenSyncConditionOperatorModel, syncCondition.operator);
        this._screenSyncConditionOperatorDropdown.connect('notify::selected-item', () => {
            const current = this._loadSyncCondition(settings);
            this._saveSyncCondition(settings, { ...current, operator: getDropDownValue(this._screenSyncConditionOperatorDropdown) || '=' });
        });
        ...
        this._screenSyncConditionValueRow = new Adw.EntryRow({ title: 'Match Value', text: syncCondition.value });
        this._screenSyncConditionValueRow.connect('changed', () => {
            const current = this._loadSyncCondition(settings);
            this._saveSyncCondition(settings, { ...current, value: this._screenSyncConditionValueRow.text });
        });
        ...
        this._screenSyncConditionCheckButton = new Gtk.Button({ label: 'Check Now', valign: Gtk.Align.CENTER });
        this._screenSyncConditionCheckButton.connect('clicked', () => void this._checkScreenSyncConditionNow());
        this._screenSyncConditionLogsButton = new Gtk.Button({ label: 'Show Logs', valign: Gtk.Align.CENTER });
    ...
    // ── Screen sync entity management ────────────────────────────────

    /** Returns lights that advertise at least one RGB-capable color mode. */
    _colorLightEntities() {
    ...
    async _checkScreenSyncConditionNow() {
        ...
        try {
            ...
            const actualValue = String(state?.state ?? '');
            const matches     = this._evaluateScreenSyncConditionValue(actualValue, condition);
            this._showScreenSyncConditionCheckDialog({ condition, actualValue, matches });
        } catch (e) {
            const message = e?.message ?? String(e);
    ...
    _rebuildScreenSyncEntityRows(settings) {
        ...
        for (let i = 0; i < configs.length; i++) {
            ...
            const removeBtn = new Gtk.Button({ label: 'Remove', css_classes: ['destructive-action'], valign: Gtk.Align.CENTER });
    ...
    // ── Transition and sensitivity helpers ───────────────────────────
    ...
    _rebuildBridgeTabSelector(settings) {
        ...
        const autoCheck = new Gtk.CheckButton({
            ...
            can_target: false, focusable: false, valign: Gtk.Align.CENTER,
        });
    ...
    _showScreenSyncConditionLogsDialog(entityId, entries) {
        const dialog = new Adw.Dialog({ title: 'State Logs', content_width: 560 });
        ...
        const scroll = new Gtk.ScrolledWindow({
            min_content_height: 320, max_content_height: 420,
        ...
        const closeButton = new Gtk.Button({
            label: 'Close', css_classes: ['suggested-action'],
    ...
    _showScreenSyncPreviewDialog(preview) {
        const dialog = new Adw.Dialog({ title: 'Screen Sync Preview', content_width: 440 });
        ...
        const swatchRow = new Gtk.Box({ orientation: Gtk.Orientation.HORIZONTAL, spacing: 8, homogeneous: true });
    ...
    _createScreenSyncPreviewSwatch(title, hex) {
        const box = new Gtk.Box({ orientation: Gtk.Orientation.VERTICAL, spacing: 4, hexpand: true });

        const [red, green, blue] = this._parsePreviewHex(hex);
        const area = new Gtk.DrawingArea({ content_width: 120, content_height: 56, hexpand: true, vexpand: false });
        ...
        const titleLabel = new Gtk.Label({ label: title, xalign: 0.5, halign: Gtk.Align.CENTER });
        titleLabel.add_css_class('caption');
        box.append(titleLabel);

        const valueLabel = new Gtk.Label({ label: hex || '—', xalign: 0.5, halign: Gtk.Align.CENTER });
